...
import { filterBlogPosts, getImageValuesFromMarkdown, getInternalLinksFromMarkdown, slugify } from './util';
...
export function getAllArtImages(
  fields = ['content', 'slug', 'tags', 'date', 'artGridIgnore'],
  options = {}
) {
  const slugs = getPostSlugs();
  let posts = slugs
    .map((slug) => getPostBySlug(slug, fields))
    // Filter false values (.DS_STORE)
    .filter((post) => post && !post.artGridIgnore)
    // sort posts by date in descending order
    .sort((post1, post2) => (post1.date > post2.date ? -1 : 1));

  const targetTags = ['Art', ...ART_SUB_TAGS]
  posts = posts.filter((post) => post.tags?.some((e) => targetTags.includes(e)));
